...
void CObjStarCount::Draw()
{
	...
	//酸素確認用カウント表示
	swprintf_s(strFis, L"の数×%2d/%d個　ボンベ回復", FiStar_Reco, FiStar_Reco_max);
	Font::StrDraw(strFis, 25, 507, 20, c);

	swprintf_s(strFos, L"の数×%2d/%d個 酸素50回復", SeStar_Reco, SeStar_Reco_max);
	Font::StrDraw(strFos, 25, 530, 20, c);

	swprintf_s(strOts, L"の数×%2d/%d個 酸素30回復", ThStar_Reco, ThStar_Reco_max);

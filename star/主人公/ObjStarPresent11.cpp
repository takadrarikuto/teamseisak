...
void CObjStarPresent11::Draw()
{
	...
	else if (crick == 2)
	{
		...
		ver++;
		Font::StrDraw(L"F型主系列星で7等星の伴星Bからなる連星である", font, size_y* ver, size, c);
		ver++;
		...
		ver++;
		Font::StrDraw(L"F型主系列星…スペクトル型がF、光度階級がVの", font, size_y* ver, size, c);
		ver++;
	...
	else if (crick == 3)
	{
		...
		ver++;
		Font::StrDraw(L"二重星の関係で、連星ではない。F型主系列星で、水素の核融合を", font, size_y* ver, size, c);
		ver++;
	...
	else if (crick == 6)
	{
		...
		ver++;
		Font::StrDraw(L"ガス殻星…赤道周囲にガスの円盤を持つことが示される恒星である", font, size_y* ver, size, c);
		ver++;
	...
	else if (crick == 7)
	{
		...
		Font::StrDraw(L"いて座に属する", font, size_y* ver, size, c);
		ver++;
		Font::StrDraw(L"いて座で3番目に明るい恒星で3等星。+3.26等級の", font, size_y* ver, size, c);
		ver++;
		Font::StrDraw(L"A2型巨星と+3.47等級のA4型準巨星から構成される", font, size_y* ver, size, c);
		ver++;
		Font::StrDraw(L"連星系で2つの星は平均距離13.4auの軌道を", font, size_y* ver, size, c);
		ver++;
		Font::StrDraw(L"21.075年の周期で互いを周回している", font, size_y* ver, size, c);
		ver++;
		Font::StrDraw(L"100年余りの固有運動の観測結果から", font, size_y* ver, size, c);
		ver++;
		Font::StrDraw(L"見かけの二重星であると考えられている", font, size_y* ver, size, c);
		ver++;
		Font::StrDraw(L"巨星…同じ表面温度を持つ主系列星よりも半径および明るさが", font, size_y* ver, size, c);
		ver++;
		Font::StrDraw(L"非常に大きい恒星のことである", font, size_y* ver, size, c);
	...
	else if (crick == 10)
	{
		...
		ver++;
		Font::StrDraw(L"地球からの距離は同じとは限らず、空間的にはまとまっていない", font, size_y * ver, size, c);
		ver++;
		Font::StrDraw(L"", font, size_y * ver, size, c);
		ver++;
	}

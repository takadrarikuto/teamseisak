...
void CObjStarPresent11::Draw()
{
	...
	else if (crick == 5)
	{
		...
		if (page == 0)
		{
			Font::StrDraw(L"いて座に属する恒星で3等星。", left_end, IO_y + Interval_y * ver, font_size, c);
			ver++;
			Font::StrDraw(L"星間物質がどの程度あるかわかっていないため不確かではあるが", left_end, IO_y + Interval_y * ver, font_size, c);
			ver++;
			Font::StrDraw(L"観測されているよりも光度は30%半径は15%ほど大きいものと", left_end, IO_y + Interval_y * ver, font_size, c);
			ver++;

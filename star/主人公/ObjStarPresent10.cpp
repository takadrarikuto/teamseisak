...
void CObjStarPresent10::Draw()
{
	...
	//					　　	    X　        Y　    大きさ
	Font::StrDraw(L"ポリマ", 20, 340 + t * l, 30, c);
	l++;
	// left				 right            top            bottom               
	if (m_mou_x > side_a + side_b * std && m_mou_x < side_b * (std + 1) && m_mou_y>300 + k * (l - 1) && m_mou_y < 300 + k * l)
	{
	...
	//					　　	  X　        Y　    大きさ
	Font::StrDraw(L"ミネラウバ", 20, 350 + t * l, 30, c);
	l++;
	// left				 right            top            bottom               
	if (m_mou_x > side_a + side_b * std && m_mou_x < side_b * (std + 1) && m_mou_y>300 + k * (l - 1) && m_mou_y < 300 + k * l)
	{
	...
	else if (crick == 5)
	{
		...
		ver++;
		Font::StrDraw(L"非常に大きい恒星のことである巨星の半径は太陽の", font, size_y * ver, size, c);
		ver++;
		Font::StrDraw(L"10倍から100倍、明るさは10倍から1000倍である", font, size_y * ver, size, c);
		ver++;
	}
	else if (crick == 6)
	{

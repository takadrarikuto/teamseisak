...
void CObjStarPresent6::Draw()
{
	...
	//					　　	    X　        Y　    大きさ
	Font::StrDraw(L"メサルティム", 20, 330 + t * l, 30, c);
	l++;
	// left				 right            top            bottom               
	if (m_mou_x > side_a + side_b * std && m_mou_x < side_b*(std + 1) && m_mou_y>300 + k * (l - 1) && m_mou_y < 300 + k * l)
	{
	...
	//					　　	  X　        Y　    大きさ
	Font::StrDraw(L"ボテイン", 20, 340 + t * l, 30, c);
	l++;
	// left				 right            top            bottom               
	if (m_mou_x > side_a + side_b * std && m_mou_x < side_b*(std + 1) && m_mou_y>300 + k * (l - 1) && m_mou_y < 300 + k * l)
	{
	...
	//					　　	  X　        Y　    大きさ
	Font::StrDraw(L"リリィブロア", 20 + 190, 320 + t * l, 30, c);
	l++;
	// left				 right            top            bottom               
	if (m_mou_x > side_a + side_b * std && m_mou_x < side_b*(std + 1) && m_mou_y>300 + k * (l - 1) && m_mou_y < 300 + k * l)
	{
	...
	{
		Font::StrDraw(L"おひつじ座に属する", 110, 0, 30, c);
		Font::StrDraw(L"太陽の3.2倍の質量を持つB型主系列星を主星とする", 110, 50, 30, c);
		Font::StrDraw(L"分光連星で、伴星は主星から軌道を", 110, 100, 30, c);
		Font::StrDraw(L"約30年掛けて周回していると考えられている", 110, 150, 30, c);
		Font::StrDraw(L"分光連星…スペクトルの特徴の周期的な変化に", 110, 200, 30, c); 
		Font::StrDraw(L"より2つ以上の天体からなることを検出できる連星", 110, 250, 30, c);
	}
